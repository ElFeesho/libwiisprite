...
namespace wsp {
    Sprite::Sprite() : Layer(),
                       _rotation(0.0f), _stretchWidth(1.0f), _stretchHeight(1.0f), _alpha(0xff), _image(nullptr), _trans(TRANS_NONE), _colRect(nullptr),
                       _frame(0), _frameRawCount(0), _frameSeq(nullptr), _frameSeqLength(0), _frameSeqPos(0),
    ...
    void Sprite::SetImage(Image *image, u32 frameWidth, u32 frameHeight) {
        ...
        if (_image != nullptr && _image->GetHeight() == image->GetHeight() &&
        ...
        _colRect->x = 0;
        _colRect->y = 0;
        _colRect->width = _width;
        _colRect->height = _height;

        // Now set framedata
        _frame = 0;
        _frameRawCount = (image->GetWidth() / _width) * (image->GetHeight() / _height);
        ...
        _refPixelX = (f32) _width / 2;
        _refPixelY = (f32) _height / 2;
        _refWidth = _refPixelX;
        _refHeight = _refPixelY;
    ...
    bool Sprite::CollidesWith(const Sprite *sprite, bool complete) const {
        ...
        cosa = cos(angle2),
                sina = sin(angle2);
        temp = rect[0].x;
        rect[0].x = temp * cosa + rect[0].y * sina;
        rect[0].y = -temp * sina + rect[0].y * cosa;
        ...
        rect[2].x += rect[1].width;
        rect[2].y += rect[1].height;

        cosa = cos(angle1 - angle2);
        sina = sin(angle1 - angle2);
        // Calculate the points of this sprite
        rect[3].x = -rect[0].height * sina;
        rect[3].width = rect[3].x;
        temp = rect[0].width * cosa;
        rect[3].x += temp;
        rect[3].width -= temp;
        rect[3].y = rect[0].height * cosa;
        rect[3].height = rect[3].y;
        temp = rect[0].width * sina;
        rect[3].y += temp;
        rect[3].height -= temp;
        ...
        if (temp < 0) {
            temp = rect[3].x;
            rect[3].x = rect[3].width;
            rect[3].width = temp;
            temp = rect[3].y;
            rect[3].y = rect[3].height;
            rect[3].height = temp;
        }

        // Verify that B is the vertex closest to the left
        if (sina < 0) {
            rect[3].width = -rect[3].width;
    ...
    bool Sprite::CollidesWith(const TiledLayer *tiledlayer) const {
        if (tiledlayer == nullptr ||
            ...
            tiledlayer->GetCellWidth() == 0 || tiledlayer->GetCellHeight() == 0)
            return false;
    ...
    void Sprite::SetFrameSequence(u32 *sequence, u32 length) {
        if (sequence == nullptr || length == 0)return;
    ...
    void Sprite::_CalcFrame() {
        ...
        if (_width == 0 || _image == nullptr || !_image->IsInitialized() || _image->GetWidth() == 0)return;
