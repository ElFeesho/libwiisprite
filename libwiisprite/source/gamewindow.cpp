...
namespace wsp {
    ...
    Image *GameWindow::_lastimage = nullptr;
    bool GameWindow::_lastimagebilinear = false;

    GameWindow::GameWindow() : _frameBuffer{}, _fb{0}, _first{true}, _rmode{nullptr}, _gp_fifo{nullptr} {
        if (_initialized)return; // Don't mess with me!
        _frameBuffer[0] = nullptr;
        _frameBuffer[1] = nullptr;
    }
    ...
    void GameWindow::InitVideo() {
        ...
        if (_frameBuffer[0] == nullptr || _frameBuffer[1] == nullptr) {
            exit(0);
        }
        ...
        VIDEO_Flush();
        VIDEO_WaitVSync();
        if ((_rmode->viTVMode & VI_NON_INTERLACE) != 0u) {
            VIDEO_WaitVSync();
        }
        _fb ^= 1;
        ...
        GX_SetDispCopyDst(_rmode->fbWidth, (u16) xfbHeight);
        GX_SetCopyFilter(_rmode->aa, _rmode->sample_pattern, GX_TRUE, _rmode->vfilter);
        // Use these values for GetWidth() and GetHeight()
        _width = (u32) _rmode->fbWidth;
        _height = (u32) _rmode->efbHeight;

        // Some additional Init code
        if (_rmode->viHeight == 2 * _rmode->xfbHeight) {
            GX_SetFieldMode(_rmode->field_rendering, GX_ENABLE);
        } else {
            GX_SetFieldMode(_rmode->field_rendering, GX_DISABLE);
        }
    ...
    void GameWindow::StopVideo() {
        ...
        free(MEM_K1_TO_K0(_frameBuffer[0]));
        free(MEM_K1_TO_K0(_frameBuffer[1]));
        free(_gp_fifo);
